...
*Setup instructions for a Gitian build of TOSC using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TOSC
...
Clone the git repositories for tosc and Gitian and then checkout the tosc version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tosc repository
...
To build TOSC (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the tosc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tosc/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tosc-project/tosc
...
For example:
```bash
URL=https://github.com/user/tosc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tosc=${COMMIT} --url tosc=${URL} ../tosc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tosc=${COMMIT} --url tosc=${URL} ../tosc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tosc=${COMMIT} --url tosc=${URL} ../tosc/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tosc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tosc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tosc-build.assert
```
...
[tosc/gitian.sigs](https://github.com/tosc-project/gitian.sigs.tosc/) repository, or if that's not possible create a pull
request. 
